...
export default function AssessComprehensiveActivityModal({
    ...
    const calculateTotalScore = () => {
        let totalScore = 0;
        for (const question of assessActivity.comprehensionTask.questions) {
            if (scores[question._id] === undefined) {
                return "N/A";
            }
        }
        for (const question of assessActivity.comprehensionTask.questions) {
            let scorePerWord = parseInt(scores[question._id]);
    ...
    const handleSubmit = (e) => {
        ...
        if (score === "N/A") {
            ...
            Swal.fire({
                title: "Are you sure?",
                text: `Do you really want to finish the assessment for ${assessActivity?.patient?.firstName} ${assessActivity?.patient?.lastName}?`,
                icon: "warning",
                ...
                if (result.isConfirmed) {
                    ComprehensionTaskService.updateActivityById(
                        assessActivity._id,
                        {
            ...
            <div className="bg-white rounded-xl relative w-full max-w-3xl mx-4 sm:mx-8 lg:w-2/3 xl:w-1/2 overflow-auto max-h-full p-4 sm:p-6 lg:p-8">
                ...
                <div className="font-montserrat pt-4">
                    <form className="space-y-6" onSubmit={handleSubmit}>
                        <div className="mt-6">
                            <p className="mb-4">
                                {assessActivity.comprehensionTask.passage}
                            </p>
                            <div className="space-y-4">
                                {assessActivity.comprehensionTask.questions.map(
                                    (question, index) => (
                                        <div
                                            key={question._id}
                                        ...
                                        >
                                            ...
                                            {/* Audio Playback */}
                                            {assessActivity.providedAnswers[
                                                index
                                            ] && (
                                                <audio
                                                    controls
