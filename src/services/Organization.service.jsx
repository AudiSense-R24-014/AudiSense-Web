...
const isTherapistAdmin = async (organizationId, therapistId) => {
  const response = await fetch(URL.ORGANIZATION_IS_ADMIN, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
...
const removeAdminPrivileges = async (organizationId, therapistId) => {
  const response = await fetch(URL.ORGANIZATION_REMOVE_ADMIN, {
    method: "DELETE",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
